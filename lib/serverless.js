...
exports.Plugin = class {
    ...
    async initialize() {
        ...
        const path = Path.join(servicePath, 'server');

        const srvModule = require(path);

        const server = this.server = await internals.getServer(srvModule);
...
exports.handler = (id, srvModule) => {

    const setup = (async () => {

        const server = await internals.getServer(srvModule);
...
internals.getServer = async (srvModule) => {

    try {

        const srv = srvModule;

        Hoek.assert(typeof srv.deployment === 'function', `No server found! The current project must export { deployment: async () => server } from server.`);
        ...
        Hoek.assert(err.code !== 'MODULE_NOT_FOUND' || !err.message.includes(`'${path}'`), `No server found! The current project must export { deployment: async () => server } from server.`);
...
exports.handler = Lalalambda.handler('${id}', server);
`;
